...
def gerar_pergunta():
    ...
    examinadores = list(banca.keys())
    examinador = examinadores[contador_perguntas % len(examinadores)]
...
def avaliar_resposta():
    ...
    try:
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {"role": "system",
                 "content": "Você é um avaliador acadêmico. Melhore a resposta fornecida otimizando clareza, coesão e argumentação."},
                {"role": "user", "content": resposta}
            ],
            temperature=0.7
        )
        resposta_melhorada = response["choices"][0]["message"]["content"].strip()
...
def copiar_resposta():
    ...
    root.clipboard_append(resposta_melhorada_text.get("1.0", tk.END).strip())
    root.update()
...
tk.Label(root, text="Simulação da Banca Examinadora", font=("Times New Roman", 14, "bold"), fg="blue").pack(pady=10)
pergunta_label = tk.Label(root, text="Clique em 'Gerar Pergunta' para iniciar", font=("Times New Roman", 12),
...
resposta_revisada_label = tk.Label(root, text="Resposta revisada aparecerá aqui", font=("Times New Roman", 12),
                                   wraplength=550, justify="left")
...
btn_copiar_melhorada = tk.Button(root, text="Copiar Resposta Melhorada", command=copiar_resposta,
